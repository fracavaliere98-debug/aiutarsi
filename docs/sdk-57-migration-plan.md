...
3. **`@react-navigation/*` (VERIFICATO al gradino 55→56, correzione rispetto all'ipotesi iniziale).** Confermato: da SDK 56 `expo-router` non dichiara più `@react-navigation/*` come propria dipendenza (`npm ls` e grep su `node_modules/expo-router/package.json` dopo l'install), ma il suo codice interno (`useNavigation.js`, `navigationParams.js`, ecc.) importa comunque `@react-navigation/native` in modo statico (bare import), non vendorizzato. **La fix corretta è l'opposto di quanto ipotizzato: `@react-navigation/bottom-tabs` va TENUTO come dipendenza esplicita in `package.json`** (oggi lo è già, usato anche da `hooks/useBottomScreenInset.ts`), perché è l'unica fonte che porta `@react-navigation/native`/`@react-navigation/core` in `node_modules` — toglierlo romperebbe la navigazione di expo-router stesso, non solo l'inset. Verificato via `npm ls @react-navigation/bottom-tabs`: istanza singola dedupata (7.19.2), nessun doppione — l'inset del tab bar funziona. Nessun codemod necessario. Unico punto scoperto: `expo-router` importa anche `@react-navigation/material-top-tabs` (non installato), ma solo nel layout Drawer/Material-Top-Tabs — l'app non li usa (grep negativo su `app/`, `components/`, `hooks/`), quindi nessun impatto pratico.
