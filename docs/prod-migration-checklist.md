...
- [ ] Smoke test edge (`npm run smoke:*:staging`, `release:smoke`): **non eseguiti il 2026-09-21** — la sandbox di sviluppo non raggiunge `*.supabase.co` via HTTP. Da lanciare dal Mac con `STAGING_SUPABASE_URL`/`STAGING_SUPABASE_ANON_KEY`. La verifica DB delle migration del 21/09 è stata invece fatta via SQL in transazioni con rollback (RLS/trigger/RPC). Edge function riallineate: sorgente repo == staging == prod per le 6 funzioni driftate (verificato 21/09 confrontando il codice; l'`ezbr_sha256` non è confrontabile tra progetti perché il bundle cambia con il layout dei path).
